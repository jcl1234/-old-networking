...
ui.Menu = class({
    ...
    mousepressed = function(self, item)
        ...
        if not ui.mode then
            if self.headMenu(self).draggable then
...
ui.Button = class({
    ...
    _init = function(self, x, y, width, height, parent)
        ...
        self.hoverFill = false
        self.hoverOutline = true
        self.hovered = false
    ...
    draw = function(self)
        if not self.transparent then
            --Hovered
            if self.hoverCol and self == ui.hovered and ui.mode == nil then
            ...
            if self.clickColor and self == ui.hovered and self == ui.clickItem and ui.mode ~= "dragging" then
                ...
                love.graphics.rectangle("line", self.x - self.border, self.y - self.border, self.width + self.border * 2, self.height + self.border * 2)
            end
...
ui.Textbox = class({
    ...
    createButton = function(self)
        ...
        local oldDraw = self.textButton.draw
        local txtBt = self.textButton
        self.textButton.draw = function(txtBt)
    ...
    textinput = function(self, key)
        ...
        if (string.len(self.text) <= self.limit) or self.limit == 0 then
            if allowed then
                ...
                if self.onType then self.onType(key) end
            end
        end
    end,
    ...
    keypressed = function(self, key)
        ...
        --Enter
        if key == "return" and self.selected then
            if self.onEnter and self.text ~= "" and self.text ~= "" then
